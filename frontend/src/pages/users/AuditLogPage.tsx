  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-text">لاگ‌های ممیزی</h1>
                  ...
                  {data.data.map((log) => (
                    <tr key={log.id} className="hover:bg-surface2 transition-colors">
                      ...
                      <td className="table-td">
                        {log.user_email ? (
                          <span className="text-sm text-text">{log.user_email}</span>
                        ) : (
                          <span className="text-xs text-text-muted italic">سیستم</span>
                        )}
                      </td>
                      <td className="table-td">
                        <span className="text-xs font-mono bg-surface2 px-1.5 py-0.5 rounded text-text-secondary">
                          {log.entity_type}
                        </span>
                      </td>
                      <td className="table-td text-xs text-text-muted font-mono">
                        {log.entity_id ?? '—'}
                      </td>
                      <td className="table-td text-xs text-text-muted font-mono">
                        {log.ip_address ?? '—'}
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-6 z-50 bg-surface border border-border rounded-xl shadow-xl p-4 w-80">
            <p className="text-xs font-semibold text-text-secondary mb-2">تغییرات</p>
            {oldVal !== undefined && oldVal !== null && (
              <div className="mb-2">
                <p className="text-xs text-danger font-medium mb-1">قبل:</p>
                <pre className="text-xs bg-danger-bg p-2 rounded-lg overflow-auto max-h-24 text-danger">
            ...
            {newVal !== undefined && newVal !== null && (
              <div>
                <p className="text-xs text-success font-medium mb-1">بعد:</p>
                <pre className="text-xs bg-success-bg p-2 rounded-lg overflow-auto max-h-24 text-success">
